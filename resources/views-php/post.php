...
<nav class="bg-white border-b">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        <ol class="flex items-center space-x-2 text-sm">
            <li>
                <a href="/" class="text-gray-500 hover:text-secondary transition-colors">Home</a>
            </li>
            <li class="text-gray-400">/</li>
            <li>
                <a href="/category/<?= strtolower(htmlspecialchars($post['category'])) ?>" class="text-gray-500 hover:text-secondary transition-colors"><?= htmlspecialchars($post['category']) ?></a>
            </li>
            <li class="text-gray-400">/</li>
...
<article class="py-12">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <!-- Category Badge -->
        <a 
            href="/category/<?= strtolower(htmlspecialchars($post['category'])) ?>"
        ...
        <div class="flex items-center space-x-4 text-gray-600 mb-8">
            <div class="flex items-center space-x-2">
                ...
                <div>
                    <p class="font-medium text-primary"><?= htmlspecialchars($post['author']) ?></p>
        ...
        <div class="border-t border-b py-6 my-10">
            <div class="flex items-center justify-between flex-wrap gap-4">
                ...
                <div class="flex items-center space-x-4">
                    <a 
                        href="https://twitter.com/intent/tweet?url=<?= urlencode('http://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI']) ?>&text=<?= urlencode($post['title']) ?>" 
                        target="_blank"
                    ...
                    <a 
                        href="https://www.linkedin.com/shareArticle?mini=true&url=<?= urlencode('http://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI']) ?>&title=<?= urlencode($post['title']) ?>" 
                        target="_blank"
                        rel="noopener noreferrer"
                        class="w-10 h-10 bg-gray-100 rounded-full flex items-center justify-center text-gray-600 hover:bg-secondary hover:text-white transition-colors"
                    >
